...
Для того, чтобы скопировать объект, надо либо сериализовать в JSON и распарсить (`var copy = JSON.parse(JSON.serialize(someObject));`), либо воспользоваться методом
